<template>
	<view class="calendar-box">
		<view class="top">
			<view>
				<h4>{{$t('check-in')}} <span> {{signDays}} </span> {{$t('days')}}</h4>
				<!-- <p>今日获得+7积分</p> -->
			</view>
			<!-- <view class="rig">
				<span>补签</span>
		...
		<view class="miss_box">
			<view class="month">
				...
				<view class="prv" @click="nextMonth">
					<span v-show="next">
						<u-icon name="arrow-right" color="#333" size="36"></u-icon>
					</span>
				</view>
		...
		<view class="but">
			<button :disabled='thisMonth == month ? false : true'  :style="'background:'+(thisMonth == month ? bgday : '#666')+';'" @click="daySign(dayArr[aheadDay + day - 1])">{{$t('punchIn')}}</button>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		mounted() {
			...
			} else if(that.lang == 'zh-TW') {
				that.weekArr = ['日', '壹', '二', '三', '四', '五', '六']
			} else if(that.lang == 'zh-CN') {
				that.weekArr = ['日', '一', '二', '三', '四', '五', '六']
			}
			// 今天周几
